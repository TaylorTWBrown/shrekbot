        ...
        if (Object.keys(tvResults.data).length > 0) {
            ...
            function showBuilder() {
                try {
                    const embed = new Discord.RichEmbed()
                        ...
                        .setURL('https://thetvdb.com/?tab=series&id=' + tvShows[position].id)
                        .setDescription(tvShows[position].description == undefined ? 'No description' : tvShows[position].description.substr(0, 255) + '(...)')
                        .setImage('https://artworks.thetvdb.com' + tvShows[position].image)
                        .setTimestamp()
            ...
            function showSelection() {
                var embededMessage = message.channel.send(showBuilder()).then(async sentEmbed => {
                    if (maxNumberOfShows == 1 && tvShows.length == 1) {
